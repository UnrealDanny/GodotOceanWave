...
func init_gpu(num_cascades : int) -> void:
	# --- MEMORY FLUSH ---
	# This prevents the @tool script from holding onto broken descriptor sets
	if context:
		context.free()
	pipelines.clear()
	descriptors.clear()
	...
	var num_fft_stages := int(log(float(map_size)) / log(2.0))
	...
	# These MUST be separated so Vulkan knows which phase is Read-Only vs Write-Only
	var spectrum_set := context.create_descriptor_set([descriptors[&'spectrum']], spectrum_compute_shader, 0)
	
	var spectrum_modulate_set_0 := context.create_descriptor_set([descriptors[&'spectrum']], spectrum_modulate_shader, 0)
	var spectrum_modulate_set_1 := context.create_descriptor_set([descriptors[&'fft_buffer']], spectrum_modulate_shader, 1)
	...
	var unpack_set_0 := context.create_descriptor_set([descriptors[&'displacement_map'], descriptors[&'normal_map']], fft_unpack_shader, 0)
	var unpack_set_1 := context.create_descriptor_set([descriptors[&'fft_buffer']], fft_unpack_shader, 1)

	# --- COMPUTE PIPELINE CREATION ---
	pipelines[&'spectrum_compute'] = context.create_pipeline([map_size/16, map_size/16, 1], [spectrum_set], spectrum_compute_shader)
	pipelines[&'spectrum_modulate'] = context.create_pipeline([map_size/16, map_size/16, 1], [spectrum_modulate_set_0, spectrum_modulate_set_1], spectrum_modulate_shader)
	...
	pipelines[&'fft_unpack'] = context.create_pipeline([map_size/16, map_size/16, 1], [unpack_set_0, unpack_set_1], fft_unpack_shader)
...
func JONSWAP_alpha(wind_speed := 20.0, fetch_length := 550e3) -> float:
	return 0.076 * pow(pow(wind_speed, 2.0) / (fetch_length * G), 0.22)

func JONSWAP_peak_angular_frequency(wind_speed := 20.0, fetch_length := 550e3) -> float:
